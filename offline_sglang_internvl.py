...
if __name__ == '__main__':
    ...
    print(vlm_model)
    vision = vlm_model.vision_model.cuda()
    mlp = vlm_model.multi_modal_projector.cuda()
    # image = Image.open(
    #     BytesIO(
    #         requests.get(
    ...
    #     )
    # )
    pixel_values = load_image(BytesIO(
            requests.get(
                "https://github.com/sgl-project/sglang/blob/main/test/lang/example_image.png?raw=true"
            ).content
    ...
    processed_prompt = tokenizer(
        conv.get_prompt(), return_tensors="pt"
    ).to(torch.bfloat16)
    input_ids = processed_prompt["input_ids"][0].detach().cpu().tolist()
    precomputed_embeddings = vision(pixel_values).last_hidden_state
    print("image features shape:", precomputed_embeddings.shape)

    precomputed_embeddings = mlp(precomputed_embeddings)
    print("projected image features shape:", precomputed_embeddings.shape)
